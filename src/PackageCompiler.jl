...
export create_sysimage, create_app, create_library, run_precompiles
...
function get_precompile_code(precompile_files::Vector{String}, extra_precompiles::String="")
    return """
...
"""
    run_precompiles(precompile_file::Vector{String})

Runs precompile statements from `precompile_files` in the current julia process.
The files listed in `precompile_files` should be generated by running juila with
`--trace-compile` flag.

### Arguments:

- `precompile_statements_file::Union{String, Vector{String}}`: A file or list of
  files that contain precompilation statements that should be executed by the current process.

"""
function run_precompiles(precompile_files::Vector{String})
    precompile_code = get_precompile_code(precompile_files)
    eval(Meta.parse(precompile_code))
end
...
function create_sysimg_object_file(object_file::String,
    ...
    precompile_code = get_precompile_code(precompile_files; extra_precompiles)
